...
from data_utils.seal_loader import SEALDataset, SEALDynamicDataset
from graphgps.network.heart_gnn import DGCNN, SAGE_Variant, GCN_Variant, GIN_Variant
from graphgps.encoder.seal import do_edge_split, do_ogb_edge_split
from graphgps.utility.utils import Logger
...
def test():
    ...
    neg_train_pred = train_true[train_true==0]
    ...
    if args.eval_metric == 'hits':
        ...
        results = evaluate_ogb_rocauc(pos_train_pred, neg_train_pred, pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred)
    elif args.eval_metric == 'auc':
        results = evaluate_auc(train_true, train_pred, val_pred, val_true, test_pred, test_true)
...
def evaluate_hits(pos_train_pred, neg_train_pred, pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred):
    results = {}
    for K in [20, 50, 100]:
        evaluator.K = K
...
def evaluate_auc(train_true, train_pred, val_pred, val_true, test_pred, test_true):
   
    train_auc = roc_auc_score(train_true, train_pred)
    valid_auc = roc_auc_score(val_true, val_pred)
    test_auc = roc_auc_score(test_true, test_pred)
    return {'AUC': (train_auc, valid_auc, test_auc)}

def evaluate_ogb_rocauc(pos_train_pred, neg_train_pred, pos_val_pred, neg_val_pred, pos_test_pred, neg_test_pred):
...
if args.dataset.startswith('ogbl'):
    ...
    split_edge = do_ogb_edge_split(dataset, args.fast_split)
    data = dataset[0]
...
# SEAL.
path = './generated_dataset' + '_seal{}'.format(args.data_appendix)
...
for run in range(args.runs):
    if args.model == 'DGCNN':
        ...
        model = SAGE_Variant(args.hidden_channels, args.num_layers, max_z, train_dataset,  
                     args.use_feature, node_embedding=emb).to(device)
    elif args.model == 'GCN':
        model = GCN_Variant(args.hidden_channels, args.num_layers, max_z, train_dataset, 
                    args.use_feature, node_embedding=emb).to(device)
    elif args.model == 'GIN':
        model = GIN_Variant(args.hidden_channels, args.num_layers, max_z, train_dataset, 
                    args.use_feature, node_embedding=emb).to(device)
            ...
            if epoch % args.log_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    to_print = (f'Run: {run + 1:02d}, Epoch: {epoch:02d}, ' +
                                f'Loss: {loss:.4f}, Train: {100 * train_res:.2f}%, ' +
