...
class LMTrainer():
    def __init__(self, cfg):
        ...
        # Preprocess data
        data, labels = load_lp_data(dataset=self.dataset_name, use_text=True, use_gpt=cfg.lm.train.use_gpt, seed=self.seed)
        data, num_classes, text = load_data(
            dataset=self.dataset_name, use_text=True, use_gpt=cfg.lm.train.use_gpt, seed=self.seed)
        
        # TODO load link prediction task from this line
        tokenizer = AutoTokenizer.from_pretrained(self.model_name)
        X = tokenizer(data, padding=True, truncation=True, max_length=512)

        dataset = CustomPygDataset(X, labels)
    ...
    def eval_and_save(self):
        ...
        # TODO optimize model architecture
